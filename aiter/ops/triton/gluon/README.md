...
- **`bh16bn64`** (`nhead &le; 16`, bf16 KV): BLOCK_H=16, BLOCK_N=64, 2-D grid `(batch_size, NUM_KV_SPLITS)` with `NUM_KV_SPLITS = max(1, 256 // batch_size)`. Use when KV is kept in bf16 (no fp8 quant). Same `NHEAD < BLOCK_H` masking. When `NUM_KV_SPLITS == 1` (e.g., `batch_size >= 256`), the implementation bypasses stage-2 and writes `o` directly; `mla_decode_gluon(...)` returns `(o, final_lse)` only when `return_lse=True` and the merged fp32 LSE is computed locally, otherwise the wrapper writes `o` directly without a separate stage-2 reduction. When `NUM_KV_SPLITS > 1`, full decode with stage-2 reduce into `o`.
